...
from '../../actions/login';
import {
    actionCateDetailPost,actionCateUpdate,actionCateEditPost,actionCateDeletePost,actionClear
}
from '../../actions/action';
...
//封装组件
class ActionCateEditPage extends Component {
    componentWillMount() {
        const {
        ...
        if (login.status) {
            if(params.id){
                dispatch(actionCateDetailPost(params.id));
            }
        }
    }
                    ...
                    <form className="form form-normal" onSubmit={e => this.handleSubmit(e)}>
                        <div className="form-control">
                            ...
                            <div className="form-control-content">
                                <input type="text" placeholder="请输入名称" ref="name" value={action.cate_detail.name} onChange={()=>this.handleChange()} />
                        ...
                        <div className="form-control">
                            <label>排序:</label>
                            <div className="form-control-content">
                                <input type="number" placeholder="请输入排序" ref="order" value={action.cate_detail.order} onChange={()=>this.handleChange()} />
                                <p className="text-tip">排序越大越靠后</p>
                            </div>
                        </div>
                        <p className="form-btn">
                            <button type="submit" className="btn btn-primary btn-lg">编辑</button>
                            {action.cate_detail._id?<button type="button" className="btn btn-danger btn-lg m-l" onClick={()=>this.handleDelete()}>删除</button>:''}
                        </p>
                    </form>
                </div>
                {action_modal}
    ...
    handleSubmit(e){
        ...
        dispatch(actionCateEditPost(action.cate_detail,()=>{
            dispatch(goBack());
        }));
    }
    handleChange(){
    ...
    handleConfirm(type){
        ...
        dispatch(actionCateDeletePost(action.cate_detail._id,()=>{
            dispatch(goBack());
        }))
    }
}
export default connect(propMap)(ActionCateEditPage);